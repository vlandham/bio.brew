local version="2.1.0"
local type="tar.gz"
local URL="http://downloads.sourceforge.net/project/expat/expat/${version}/expat-${version}.${type}"
local tb_file=`basename $URL`
local seed_name="expat-${version}"
local install_files=(install/bin/xmlwf)

do_install()
{
  cd $TB_DIR
  download $URL $tb_file
  decompress_tool $tb_file $type
  mv $TB_DIR/$seed_name $STAGE_DIR/$seed_name
  cd $STAGE_DIR/$seed_name
  configure_tool $seed_name "" "$STAGE_DIR/$seed_name/install"
  make_tool $seed_name $make_j
  install_tool $seed_name
}

do_activate()
{
  link_from_stage $seed_name ${install_files[@]}
  link_library $seed_name 'install/lib'
  link_include $seed_name 'install/include'
  link_share $seed_name 'install/share'
}

do_test()
{
	log "test"
}

do_remove()
{
  remove_recipe $seed_name
  remove_from_stage $seed_name ${install_files[@]}
}

source "$MAIN_DIR/lib/case.sh"
